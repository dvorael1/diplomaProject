...
# Created: May 7, 2012
# Modified:
#   June 20, 2013.  Add (0,0) and (1,1) if FPR has only one unique value
#   Jan 7, 2015. Added plot.ROC.with.AUC() to deal with undefined precision values
#   Feb 10, 2019. Modified plot.ROC.with.AUC() to extract a new function
#     calculate.ROC.PR(), so that it can be reused in AUC-multiple.R

trapezint <- function (x, y, a, b) {
  ## This function is copied exactly from the ROC package version 1.32.0
  ## It assumes that x is sorted.  MS. 5/7/2012
